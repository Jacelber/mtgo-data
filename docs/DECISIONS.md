...
# DEC-150 - Restart one MTGO production run when its publication base becomes stale

Status: `Accepted`

## Context

The MTGO production workflow builds and validates an immutable candidate from
its trigger commit, then publishes one generated-data commit directly to
`master`. Its non-cancelling concurrency group serializes production runs but
does not prevent an independently approved pull request from merging while a
candidate is being generated.

Runs `32832640536`, `32954197203`, and `33316435621` each passed candidate
generation and validation, then failed `git push origin HEAD:master` with a
non-fast-forward rejection after a production-relevant change reached
`master`. DEC-031 intentionally rejected rebasing output generated by older
code, but repeated safe failures leave a valid scheduled refresh unpublished
and require a manual replacement run.

## Decision

Keep the immutable-source and fail-closed publication rule. Before downloading
the validated output in the publish job, resolve the current remote `master`.
If it differs from the run's source commit, do not restore, commit, rebase, or
push that candidate. If `master` changes after this check and the normal push is
rejected, resolve the remote tip again and classify the same condition as a
stale publication base. Authentication, transport, or other push failures with
an unchanged remote tip remain ordinary publish failures.

When the generated-data push succeeds but `master` advances before the
confirmation step, accept the local publication commit only if it remains an
ancestor of the observed remote tip. The existing Pages admission then binds
and compares the exact publication evidence; diverged history still fails.

The first stale run may request exactly one new `update.yml` dispatch on the
current `master`. It passes only its parent run ID; it does not pass or reuse the
old fetched candidate, generated output, baseline, or validation evidence. The
replacement therefore repeats the normal fetch, generation, candidate
validation, generated-page smoke, and publication gates under its own source
commit. If that replacement also becomes stale, it fails closed and requests no
third run.

Retain the existing `publish` failure-Issue marker so repeated incidents remain
deduplicated. Add only the controlled failure kind, source commit, observed
remote commit, bounded-recovery result, and parent run ID. Do not copy raw logs
or source responses, and do not close the Issue automatically after recovery.
Keep the existing schedule, concurrency group, job permissions, artifact
retention, candidate boundary, Pages evidence, and downstream success gates.

## Consequences

An ordinary unchanged-base production run behaves as before. A stale run is
visible as failed and cannot produce readiness or Pages output, while one fresh
run waits behind it in the existing concurrency group. Concurrent repository
work is never overwritten, old generated output is never attached to newer
code, and a persistent stream of `master` changes cannot create an unbounded
workflow loop.

This decision changes production recovery orchestration only. It does not
