...
新预览标记 `renderer_selection: {method: entry-direct-v1, entry: index.html}`，仅收录
MTGO 根入口直接声明的本地 script src / stylesheet href，并绑定入口 HTML 本身。
实际加载的 renderer JS/CSS 继续按完整文件绑定；`archetype-visuals.js` 保持现有
展示颜色、代表牌和已选图片的语义级处理，不重新绑定整个跨赛制配置文件。
路径越界、资源缺失、外部 URL、query/fragment、base URL 或歧义声明会明确拒绝。
这不是完整运行时依赖图，不递归解析 import、Worker、CSS @import 或共享脚本内部。

旧集合只有在旧/新入口摘要相同、当前直接资源在旧证据中完整存在且摘要相同，
并且维度和 bindings 内的资源映射一致时，才允许比较时去掉旧集合的额外资源。
其余内容、颜色、已选图片及原本受保护的绑定继续比较。入口/实际资源改变返回 changed；
缺失、不一致或不支持的选择证据返回 evidence_required。旧 packet 与原决定不改写。

普通决定复用、resume、归档完成以及 supplement 沿用和历史校验共用 preview_validity。
普通提交保留完整绑定比较，完成查询保留既有页面维度比较合同，两者共享相同资源证据
核对。比较兼容不替代原包完整性、发布操作、health 或范围校验。W38 历史不迁移。
