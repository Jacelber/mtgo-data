...
| `tests/test_classifier_closure.py` | Classifier closure discovery, complete-public selection, provenance, or format-atomic materialization changes | Prove private executable formats are rejected before output, plus minimal binding, exact SHA closure, and rollback without live corpus or fetch | Synthetic public/private registry, name, Melee chain, and two-file transaction subjects | `synthetic` |
...
| `tests/test_review_workbook.py` | Shared Owner-review XLSX reader, intake command, or workbook-governance changes | Resolve values by raw OOXML semantics, reject invalid indexes, and report only normalized cell differences | Synthetic OOXML carriers with content-derived shared-string positions | `synthetic` |
...
| `tests/test_mtgo_reviewed_publication.py` | MTGO reviewed publication membership or admission changes | Keep data admission separate from completion and preserve continuous per-format publication | Synthetic initial scope, weekly admissions, empty weeks, and late events | `synthetic` |
| `tests/test_mtgo_review_scope.py` | Explicit private review scope, private Landing, name/workbook binding, readiness, completion, or format-scoped Schema mapping changes | Preserve independent closed weeks, registry and path identity, private/public isolation, and historical digest compatibility | Synthetic executable third format with repository-external artifacts only | `synthetic` |
| `tests/test_mtgo_third_format.py` | Shared third-format generation, complete-public qualification, Schema, production routing, catalog, or Pages admission changes | Separate planned, private executable, incomplete public, and complete public states without using the stored catalog as admission authority | One synthetic third format, fixed source records, and registry/catalog drift | `synthetic` |
...
| `tests/test_pages_candidate_measurement.py` | Pages candidate measurement packaging, evidence, or this test changes | Keep the measurement-only payload deterministic, closed, and independently verifiable | Two synthetic site files and tamper/path cases | `synthetic` |
...
| `tests/test_trusted_pages_artifact.py` | Trusted Pages cache/seed artifact selection or this test changes | Accept only unexpired artifacts from completed successful approved master Pages runs | Synthetic artifact and workflow-run provenance | `synthetic` |
...
| `tests/test_weekly_maintenance_readiness.py` | Weekly readiness or completion lifecycle changes | Preserve legacy 1.6 and independent-format 1.7 readiness, separate data admission, and legacy/full-review completion meanings | Synthetic Standard/Modern readiness, Landing, and completion subjects | `synthetic` |
...
| Format registry, shared catalog qualification, classifier-closure selection, or either admission test changes | `python -B -m pytest tests/test_mtgo_third_format.py tests/test_classifier_closure.py` with an external basetemp | Prove the four lifecycle states, registry-derived public qualification, private fail-closed behavior, and stored-catalog independence | Once on the final shared-admission tree |
...
| MTGO candidate generated | Candidate boundary, full repository validation, rules, Schema, output-invariant, and generated-consumer contracts in `update.yml` | Block publication when the candidate changes prohibited paths, violates repository or data contracts, or cannot be consumed through the deterministic generated-data interfaces | Once for that immutable candidate before packaging |
| Melee candidate generated | Candidate boundary, repository, rules, and Schema checks plus exact staged-tree binding in `fetch_melee.yml` | Block the final derived candidate commit and push unless its complete staged tree is unchanged after deterministic validation; the earlier immutable raw/normalized checkpoint remains unchanged | Once for that immutable candidate after exact staging and before its final derived commit |
...
GOV-10 retired feature-level filter, scrolling, layout, and detail assertions
from the production hard gate. DEC-170 retires the remaining generated-page
browser smoke and deletes `production-pages.spec.js`; candidate publication now
uses only the deterministic gates listed above.
