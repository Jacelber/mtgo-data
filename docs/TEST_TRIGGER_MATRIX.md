...
| Card-localization external-source integration changes after the rights gate | One separately authorized read-only build for the exact bounded product subject into an external temporary directory, followed by manifest, attribution, byte, and digest verification | Prove the admitted MTGCH snapshot fields match the documented provenance classes and that every retained real image is within the recorded redistribution decision | Once on the final `L10N-B` tree before Owner review; no public admission when rights or attribution remain ambiguous |
