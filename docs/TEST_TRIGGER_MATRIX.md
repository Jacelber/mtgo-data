...
| Card-localization identity, provenance, resolver, source adapter, or manifest contract changes | A future focused localization contract test using only synthetic official, community, fallback, double-faced, collision, unsafe-path, and permission-denied fixtures | Prove immutable identity joins, truthful provenance, deterministic official/community/English priority, atomic rejection, and zero real community image bytes before the rights gate | Once on the final `L10N-A` tree before Owner review; never contact MTGCH or an image CDN from automated tests |
| `L10N-DIRECT-TRIAL` real-source delivery diagnostic | One separately authorized ephemeral browser run from the deployed Pages origin using the accepted predeclared sample, request ceiling, pacing, cold/warm matrix, and fail-closed stop conditions | Determine whether direct MTGCH image delivery is eligible for a later architecture proposal without changing product behavior or retaining raw card responses, image bytes, screenshots, or browser state | Once after the trial contract is accepted and merged and the exact diagnostic is separately authorized; never run in CI, during contract publication, or as authorization for a product change |
| Card-localization external-source integration changes after the rights gate | One separately authorized read-only build for the exact bounded product subject into an external temporary directory, followed by manifest, attribution, source-specific original-byte, and digest verification | Prove official and community names satisfy their separate source and attribution contracts; each retained image is either an original Scryfall image for a proven official Simplified Chinese printing or an identifiable MTGCH community-rendered image covered by the Owner-recorded permission; and the bundle contains zero unpermitted submitted, third-party, further-modified, source-unknown, or raw-response bytes | Once on the final `L10N-B` tree before Owner review; no public admission when identity, source, rights, attribution, or source-specific original-byte evidence remains ambiguous |
| Chinese card-name or card-image consumer changes | Focused future Node contract checks plus local browser review with synthetic official, community, English-fallback, missing-sidecar, and rejected-sidecar fixtures | Prove Chinese views use the admitted sidecar, label fallback truthfully, never call MTGCH at runtime, and preserve the existing English image path | Once on the final `L10N-C` visible tree before Owner review at desktop, 390px, and 412px |
