...
| Matchup model, matchup labels, matchup i18n logic, or JavaScript multi-event reference changes | `node --test tests/js/phase8-matchup-model.test.js` | Catch a broken single-event calculation or cross-runtime multi-event contract before Owner review | Once on the final visible tree before Owner review |
