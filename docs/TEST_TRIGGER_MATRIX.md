...
| Melee candidate generated | Candidate boundary, repository, rules, and Schema checks, exact staged-tree binding, and the candidate-aware Tabletop test in `production-pages.spec.js` from `fetch_melee.yml` | Block the final derived candidate commit and push unless the exact staged candidate renders through the current production consumer; the earlier immutable raw/normalized checkpoint remains unchanged | Once for that immutable candidate after exact staging and before its final derived commit; this is not a generic browser/UI regression suite |
