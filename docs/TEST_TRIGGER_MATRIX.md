...
| `tests/test_landing_card_image_cache.py` | Rolling featured-card selection, Scryfall Bulk mapping, cache manifest, Pages generated-overlay admission, or their Schema/workflow changes | Prove exact four-week selection, deduplication, repository JPEG reuse, double-faced resolution, atomic failure, byte closure, and Pages overlay admission without contacting Scryfall | Synthetic Standard/Modern weeks, Bulk Data, and JPEG bytes only |
...
| Cache-A external-source integration changes | One read-only build of the current rolling subject from Scryfall Oracle Cards Bulk Data and image CDN into an external temporary directory, followed by local verification | Prove current featured names resolve and the real immutable bundle closes without writing images to Git | Once on the final Cache-A tree before Owner review; never from automated tests |
