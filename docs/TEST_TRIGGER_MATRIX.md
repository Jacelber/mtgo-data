...
| MTGO candidate generated | Candidate boundary, repository, rules, Schema, output-invariant, consumer, and the candidate-derived `production-pages.spec.js` rendering smoke in `update.yml` | Block publication when the legacy Pickup route cannot render the last admitted Landing, or when either public product fails candidate-derived filter, sticky-header, detail, and current-data rendering contracts; no assertion depends on a rolling archetype order or row position | Once for that immutable candidate before packaging |
