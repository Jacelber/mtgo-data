...
| `tests/test_classifier_closure.py` | Classifier closure discovery, provenance, or format-atomic materialization changes | Prove minimal binding, exact SHA closure, and rollback without live corpus or fetch | Synthetic name, Melee chain, and two-file transaction subjects | `synthetic` |
...
| `tests/test_mtgo_third_format.py` | Shared third-format generation, Schema, production routing, catalog, or Pages admission changes | Separate planned, private executable, incomplete public, and complete public states without real product data | One synthetic third format and fixed source records | `synthetic` |
