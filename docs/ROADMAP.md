...
| `L10N-STAGE-C-TRIAL` | Stage C still has no image status, decode, redirect, byte, latency, cache, or fallback evidence. | After runner acceptance/publication and separate source/image authorization, refresh or prove the offline subject, regenerate the deterministic exact sample, and let the Owner run both Pages-origin sessions on the same controlled local machine at least six hours apart. Retain only the aggregate result. | One conclusive result can evaluate DEC-140's architecture criteria; any setup, observability, budget, drift, or retention failure remains inconclusive. | `gpt-5.6-sol`, high reasoning |
