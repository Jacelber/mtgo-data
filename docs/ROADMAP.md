...
| `L10N-DIRECT-TRIAL` | Direct-origin and hybrid-cache architectures cannot be compared from storage estimates alone. | After separate authorization, use the real Pages origin in an ephemeral browser to measure a predeclared current-product sample under sequential, paced-concurrent, cold, and warm conditions; retain only aggregate observations and close the browser state. | Direct MTGCH delivery becomes eligible for later consideration only if the complete bounded sample decodes without blocking or transient HTTP failures and exhibits usable browser caching; otherwise the trial selects hybrid caching for the later architecture proposal. | `gpt-5.6-terra`, medium reasoning |
| `L10N-B` | The validated contract is not yet available through a sustainable real-source resource architecture. | After the direct-image trial and a separate Owner architecture decision, build the bounded real manifest and only the image payload that decision admits; validate identities, provenance, attribution, digests, and atomic fallback without mutating the existing English cache prematurely. | Chinese display resources become reproducible while image storage remains within the accepted six-format capacity and availability boundary. | `gpt-5.6-sol`, high reasoning |
...
diagnostic exception is defined by DEC-138. Neither decision authorizes a
source fetch, the diagnostic itself, or `L10N-B`; each real-source operation
requires its own exact authorization after its contract is accepted and merged.
