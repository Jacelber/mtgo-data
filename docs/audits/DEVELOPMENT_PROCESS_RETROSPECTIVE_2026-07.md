...
| Incomplete external records are treated as fatal parser breaks | Actions run 29779522176 failed before a Legacy archive published its decklist marker; run 30174718660 later returned valid Modern event JSON with required fields still missing, exposing that the first deferral fix covered only one partial-publication shape | One pending event aborted the multi-format production workflow | Retry recent, structurally parseable partial publications and defer them within a bounded grace period; keep invalid markers, invalid JSON, invalid field types, and expired partial records fatal; report deferred and failed records separately |
