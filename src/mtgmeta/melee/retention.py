...
This module never fetches data. It accepts only a complete immutable v2, v3, or v4
...
def _load_manifest(snapshot: Path, schema_path: Path) -> tuple[dict[str, Any], bytes]:
    ...
    if manifest["schema_version"] not in {"2.0.0", "3.0.0", "4.0.0"}:
        raise MeleeRetentionError(
            "production retention requires complete raw manifest 2.0.0, 3.0.0, or 4.0.0"
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Complete immutable v2, v3, or v4 snapshot",
    )
