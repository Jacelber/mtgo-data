...
def test_reference_invalid_values(tmp_path):
    ...
    for value in ("", None, 1, "../outside.md", "C:\\outside.md", "a\\b", "//server/share"):
        ...
        assert checked == 30 and len(matching) == 1 and matching[0].category == "References"
...
def test_status_structure_failure(tmp_path):
    ...
    assert checked == 27 and len(failures) == 31 and all(f.category == "References" for f in failures)
...
def test_requirement_include_forms(tmp_path):
    ...
    assert checked == 32 and not [f for f in failures if f.path.startswith("requirements-dev.txt")]
...
def test_frontend_and_standard_missing_references(tmp_path):
    ...
    assert checked == 33 and len(failures) == 37
...
def test_pickup_valid_entry(tmp_path):
    ...
    assert checked == 28 and not [f for f in failures if "pickup" in f.path]
