...
def manifest_outputs():
    manifest = json.loads(
        (ROOT / "schemas" / "manifest.json").read_text(encoding="utf-8")
    )
    matched = []
...
def test_production_public_outputs_pass():
    checked, failures = schemas.validate_manifest(
        ROOT, ROOT / "schemas" / "manifest.json"
    )
    _, matched = manifest_outputs()
    assert checked == len(matched)
    assert failures == []


def test_every_public_output_embeds_the_manifest_version():
    manifest, matched = manifest_outputs()
...
def test_all_declared_schemas_are_valid_and_versioned():
    ...
    assert "melee-event-statistics-common.schema.json" in loaded
    assert "melee-event-overview.schema.json" in loaded
...
def test_required_top_level_field_removal_fails(filename):
    ...
    mapping = next(
        item for item in manifest["mappings"] if source.match(item["pattern"])
    )
...
def test_wrong_source_and_unknown_top_level_field_fail():
    instance = json.loads(
        (ROOT / "stats/standard/mtgo/meta.json").read_text(encoding="utf-8")
    )
    loaded, registry = schemas.load_schemas(ROOT / "schemas")
    instance["source"] = "melee"
    assert schemas.validate_instance(
        instance, loaded["mtgo-meta.schema.json"], registry
    )
...
def test_matchup_schemas_require_the_hierarchical_contract(format_id):
    ...
    failures = schemas.validate_instance(
        matchup,
        loaded["mtgo-matchup.schema.json"],
        registry,
    )
    assert failures

    catalog = json.loads(
        (ROOT / f"stats/{format_id}/mtgo/matchup_index.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    failures = schemas.validate_instance(
        catalog,
        loaded["mtgo-matchup-catalog.schema.json"],
        registry,
    )
    assert failures
...
def test_manifest_rejects_missing_matches_and_schema(tmp_path):
    ...
    manifest["mappings"] = [
        {"pattern": "missing/*.json", "schema": "mtgo-meta.schema.json"}
    ]
    ...
    manifest["mappings"] = [
        {"pattern": "stats/standard/mtgo/meta.json", "schema": "missing.schema.json"}
    ]
    path.write_text(json.dumps(manifest), encoding="utf-8")
...
def test_cli_pass_help_usage_and_non_root_execution(tmp_path):
    ...
    assert (
        ...
        and f"checked={len(matched)}" in result.stdout
    )
    help_result = subprocess.run(
        [sys.executable, "-B", str(script), "--help"], text=True, capture_output=True
    )
    assert help_result.returncode == 0 and "usage:" in help_result.stdout
    usage = subprocess.run(
        [sys.executable, "-B", str(script), "--unknown"], text=True, capture_output=True
    )
...
def test_cli_content_failure_and_infrastructure_error(tmp_path):
    ...
    schema = {
        ...
        "$id": "https://example.test/value.schema.json",
        "type": "object",
        "required": ["value"],
    }
        ...
        json.dumps(
            {
                ...
                "mappings": [
                    {"pattern": "stats/value.json", "schema": "value.schema.json"}
                ],
            }
        ),
        encoding="utf-8",
    ...
    assert (
        error.returncode == 2
        and "ERROR" in error.stdout
        and "Traceback" not in error.stderr
    )
