...
def _prepare_candidate_generation(monkeypatch, tmp_path, *, digest: str) -> None:
    ...
    monkeypatch.setattr(
        screening,
        "_screening_directories",
        lambda *args, **kwargs: (tmp_path, tmp_path),
    )
    monkeypatch.setattr(screening, "load_rules_for_format", lambda *args, **kwargs: rules)
    monkeypatch.setattr(screening, "load_screening_policy", lambda *args, **kwargs: POLICY)
    monkeypatch.setattr(screening.stats, "load_all_events", lambda *args, **kwargs: EVENTS)
    monkeypatch.setattr(
        screening.stats,
        "latest_complete_week",
        lambda *args, **kwargs: WEEK_START,
    )
    monkeypatch.setattr(screening, "load_known", lambda *args, **kwargs: {"known"})
    monkeypatch.setattr(screening, "classifier_digest", lambda value: digest)
    monkeypatch.setattr(screening, "week_records", lambda *args, **kwargs: [])
    monkeypatch.setattr(
        landing,
        "machine_fact_digest_for_week",
        lambda *args, **kwargs: MACHINE_FACT_DIGEST,
    )
    monkeypatch.setattr(
        editorial,
...
def test_week_records_add_stable_source_identity(monkeypatch):
    ...
    first = screening.week_records(EVENTS, object(), WEEK_START)[0]
    second = screening.week_records(EVENTS, object(), WEEK_START)[0]
...
def test_standard_candidate_rows_include_exact_source_and_classifier_identity(
    ...
    monkeypatch.setattr(screening, "week_records", lambda *args, **kwargs: [record])
    monkeypatch.setattr(
        screening,
        "_record_identity",
        lambda *args: {
    ...
    candidates, _base, _top8_count, _deduplicated_count = (
...
def test_unreviewed_candidate_regenerates_when_classifier_digest_changes(
    ...
    result = screening.prepare_candidates(
        tmp_path,
        "standard",
    ...
    assert refreshed["machine_fact_digest"] == MACHINE_FACT_DIGEST
    assert refreshed["link_catalog_digest"] == LINK_CATALOG_DIGEST
...
def test_unreviewed_candidate_regenerates_when_selection_policy_changes(
    ...
    assert result is not None
    assert result["skipped_existing"] is False
    assert refreshed["selection_policy_digest"] == screening.document_digest(POLICY)


def test_unreviewed_candidate_regenerates_when_fact_binding_is_missing(
    monkeypatch, tmp_path
):
    _prepare_candidate_generation(monkeypatch, tmp_path, digest="a" * 64)
    candidate_path = tmp_path / "candidates_2026-W33.yaml"
    ...
    stale.update(
        {
            "source_event_ids": ["100"],
            "classifier_digest": "a" * 64,
            "selection_policy_digest": screening.document_digest(POLICY),
        }
    )
    candidate_path.write_text(yaml.safe_dump(stale), encoding="utf-8")
